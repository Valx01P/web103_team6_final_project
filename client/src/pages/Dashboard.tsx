...
// First, fix fetchUserVotes to handle 404 properly
...
const handleVote = async (type: 'post' | 'comment', id: number, positive: boolean) => {
  ...
  console.error(`Error while ${method === 'POST' ? 'creating' : method === 'PUT' ? 'updating' : 'deleting'} vote:`, error);
  }
};

const updateScoreInState = (data: any, type: 'post' | 'comment', id: number) => {
  if (type === 'post') {
    ...
    if (selectedPost?.id === id) {
      setSelectedPost(prev => (prev ? { ...prev, score: updatedItem.score } : null));
    }
